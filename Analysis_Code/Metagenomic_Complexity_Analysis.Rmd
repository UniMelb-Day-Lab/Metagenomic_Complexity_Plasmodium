...
for (plotpid in setpid) {
  ...
  for (agegrp in c("All", AgeGroups_list)) {
    for (sp in c("Pf", "Pm", "Po")) {
      if (agegrp == "All") {
        fet_tmp <- file_isolates_field_species_binary %>% filter(species==sp) %>% select(!c(Isolate, AgeGrp, species)) %>% table() %>% fisher.test(.)
      } else {
        ...
        fet_tmp <- table(factor(fet_tmp$DBS, levels=DBS_levels), factor(fet_tmp$WB100, levels=WB100_levels)) %>% fisher.test(.)
      }
...
for (plotpid in setpid) {
  ...
  stats_sex <- wilcox.test(sum_complexity ~ sex_ML, data = file_isolates_MOI_100, exact = FALSE)
  
  ## test village (Mann Whitney U)
  stats_village <- wilcox.test(sum_complexity ~ village_ML, data = file_isolates_MOI_100, exact = FALSE)
  
  ## test hgb (Spearman's)
  stats_hgb <- cor.test(x=file_isolates_MOI_100$hgb, y=file_isolates_MOI_100$sum_complexity, method = 'spearman', exact=FALSE)
  
  ## test age (Spearman's)
  stats_age <- cor.test(x=file_isolates_MOI_100$age, y=file_isolates_MOI_100$sum_complexity, method = 'spearman', exact=FALSE)
  
  ## test temp (Spearman's)
  stats_temp <- cor.test(x=as.numeric(file_isolates_MOI_100$temp), y=file_isolates_MOI_100$sum_complexity, method = 'spearman', exact=FALSE)
...
for (plotpid in setpid) {
  ...
  plot_DBS_WB <- file_otutable_freqs_prop %>%
    ggplot(., aes(x=name, y=prop, color=value, group=value, shape=value)) +
    geom_line() +
    geom_point(size=4) +
    theme_bw() +
  ...
  ##### Save output
  ## Save files in RDS format
  saveRDS(plot_DBS_WB, paste0("Z04.POPULATION.out_RDS/0", PART, ".id", plotpid, ".FIELD_POP.DBS_WB_propDBLa.LINES.PLOT.rds"))
  
  ## Save files in TIFF format
  ggsave(paste0("Z04.POPULATION.out_RESULTS/0", PART, ".id", plotpid, ".FIELD_POP.DBS_WB_propDBLa.LINE.tiff"),
         plot_DBS_WB,
         device="tiff", dpi=300,
         width=8, height=5)
